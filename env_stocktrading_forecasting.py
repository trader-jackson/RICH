...
class StockTradingEnv(gym.Env):
    ...
    def __init__(
        ...
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(self.stock_dim, self.state_space+len(self.tech_indicator_list)+2*self.hidden_channel+1)) # cov matrix list + technical list + temporal feature * 60 + prediction labels + holding amount
        self.hidden_state_space = spaces.Box(low=-np.inf, high=np.inf, shape=(self.stock_dim, self.hidden_channel+1))
